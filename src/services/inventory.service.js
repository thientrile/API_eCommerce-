...
const importInventory = async (userId, data) => {
    ...
    const checkWareExist = await checkExistWarehouse({
        _id: convertToObjectIdMongoose(warehouseId), ware_userId: convertToObjectIdMongoose(userId),
    });
    ...
    for (let item of sku_list) {
        if(item.quantity < 0) throw new BadRequestError(`the Quantity of item with sku_id:'${item.sku_id}'must be greater than 0`);
        const checkSku = await skuModel.findOneAndUpdate({
            sku_id: item.sku_id, sku_shopId: convertToObjectIdMongoose(userId)
        }, {
            $inc: {
        ...
        await inventoryMode.findOneAndUpdate({
            inv_skuId: item.sku_id, inv_warehouseId: convertToObjectIdMongoose(warehouseId),
        }, {
            $inc: {
